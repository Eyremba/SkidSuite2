...
public class MethodSimulatorPanel extends JPanel {
	...
	public static void load(MethodNode mn) {
	...
	private static String toText(AbstractInsnNode ain) {
		...
		case AbstractInsnNode.JUMP_INSN:
			...
			return base + " Offset: " + jin.label.getLabel().setIgnoreStatus(true).getOffset();
		case AbstractInsnNode.LABEL:
			LabelNode lin = (LabelNode) ain;
			return base + " Offset: " + lin.getLabel().setIgnoreStatus(true).getOffset();
		...
		case AbstractInsnNode.LOOKUPSWITCH_INSN:
			...
			for (LabelNode o : asin.labels) {
				s += o.getLabel().setIgnoreStatus(true).getOffset() + ",";
			}
			if (s.contains(",")) {
				s = s.substring(0, s.lastIndexOf(","));
			}
			return base + " Start: " + asin.dflt.getLabel().setIgnoreStatus(true).getOffset() + " Endpoints[" + s + "]";
		...
		case AbstractInsnNode.TABLESWITCH_INSN:
			...
			for (LabelNode o : tsin.labels) {
				s2 += o.getLabel().setIgnoreStatus(true).getOffset() + ",";
			}
			if (s2.contains(",")) {
